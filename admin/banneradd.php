...
$insert=mysqli_query($conn,"insert into banner (url_image, estado) values ('$imagen_demo','0')");
$sql_last=mysqli_query($conn,"select LAST_INSERT_ID(id) as last from banner order by id desc limit 0,1");
$rw=mysqli_fetch_array($sql_last);
$id_banner=intval($rw['last']);
$sql=mysqli_query($conn,"select * from banner where id='$id_banner' limit 0,1");
